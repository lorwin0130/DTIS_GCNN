...
def tensorise_smiles(smiles, max_degree=5, max_atoms=None):
    ...
    for mol_ix, s in enumerate(smiles):
        ...
        for atom_ix, atom in enumerate(atoms):
            # write atom features
            atom_tensor[mol_ix, atom_ix, : n_atom_features] = feature.atom_features(atom) ###输入结点特征
        ...
        for bond in bonds:
            # lookup atom ids
            a1_ix = rdkit_ix_lookup[bond.GetBeginAtom().GetIdx()]  ###边输入为
...
# 将蛋白质口袋张量化,返回三种张量
def tensorise_pocket(node_lst, edge_lst, max_degree=None, max_nodes=None):
    """Takes a pocket and turns the graphs in tensor representation.
    # Returns:
        atoms: np.array, An atom feature np.array of size `(molecules, max_atoms, atom_features)`
        edges: np.array, A connectivity array of size `(molecules, max_atoms, max_neighbours)`
    """
    # import sizes
    n_atom_features = 480
    ...
    atom_tensor = np.zeros((1, max_nodes or 1, n_atom_features))
    edge_tensor = -np.ones((1, max_nodes or 1, max_degree or 1), dtype=int)
    ...
    for atom_ix, atom in enumerate(atoms):
        # write atom features
        atom_tensor[0, atom_ix, : n_atom_features] = atom ###输入结点特征

    # store connectivity matrix
    for a1_ix, neighbours in enumerate(edge_lst):
        degree = len(neighbours)
        edge_tensor[0, a1_ix, : degree] = neighbours

    return T.from_numpy(atom_tensor[0]).float(), \
           T.from_numpy(edge_tensor[0]).long()